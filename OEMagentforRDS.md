...
```
 [root@ip- ~]# nc -zv <OMS HOST IP> 4903
...
> Adding the `OEM_AGENT` option doesn’t cause any downtime. However, the instance will be in a modifying state until the agent installation is complete.

---

## Verifying the OEM Agent Configuration

To confirm that the OEM Agent is successfully configured:

1. Log in to Oracle Enterprise Manager.  
2. Check the **Hosts** section in OMS for the RDS endpoint. It should be marked as **Available**.  
3. Use the `rdsadmin` PL/SQL packages to validate the agent status.
...
```
#### collect task id and use below sql to get the status
```
SELECT text FROM table(rdsadmin.rds_file_util.read_text_file('BDUMP','dbtask-<taskid>.log'));
```

3. Open OEM, navigate to **Setup > Add Target > Add Targets Manually**.
...
7. Configure other database properties such as port, protocol, and SID.  
8. Click **Test Connection** to verify the entered values, then choose **Submit** to save the target.  
